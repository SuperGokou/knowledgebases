...
9. 模型未配置、配置错误或供应商暂不可用时，聊天返回带来源的 `retrieval_fallback`，不得把失败包装成生成式回答。

`grounded` 表示来源可追溯和编号通过确定性校验；`answer_review.status=passed` 进一步表示生成后语义审核已通过。该机制显著降低幻觉风险，但模型审核不能替代高风险业务场景的人工复核，后续仍应加入内容版本/哈希、独立审核模型和离线蕴含评测集。

## 分阶段交付

- Phase 1：登录前端、聊天工作区、知识库/用户/角色/权限管理，以及服务端授权骨架。
- Phase 2a（已交付）：发布状态、授权全文检索、可选 RAG、强制来源协议与安全降级。
- Phase 2b（部分交付）：生成后语义审核闸门；Office/PDF 安全解析 Worker、混合向量检索、重排与离线引用蕴含评测仍待完成。
