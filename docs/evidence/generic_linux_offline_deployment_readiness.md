...
静态配置现在默认只把 `19443/19444` 绑定到 `127.0.0.1`，正式运行配置必须显式使用目标服务器的 VPN、企业私网或回环地址；环境预检会拒绝 `0.0.0.0`。

正式上线必须满足：

1. 把 `KB_BIND_ADDRESS` 设置为目标服务器的 VPN/企业私网地址；不得使用 `0.0.0.0` 依赖外围防火墙补救全网卡监听。
