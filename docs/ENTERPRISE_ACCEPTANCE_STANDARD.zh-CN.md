...
本标准适用于江苏和熠光显有限公司企业知识中台的代码、Web 工作台、FastAPI 控制面、PostgreSQL、Redis、对象存储、OKF、知识问答、模型管理，以及腾讯云离线部署。它是发布门禁，不是功能愿望清单；任何一项只有在产生可复核证据后才能判定通过。
...
| 腾讯云离线验收 | 8C16G/300GB、无外网、恢复、负载、升级回滚 | 批准的脱敏/合成数据集 |

腾讯云离线验收的最低硬件为 8 个逻辑 CPU、15GB 可用内存、300GB 物理磁盘、初始至少 240GB 可用空间。规格不足时必须终止，不得降低门槛后继续。
...
真实服务器命令、TLS/断网、备份恢复和负载步骤以[腾讯云离线企业部署](TENCENT_OFFLINE_ENTERPRISE_DEPLOYMENT.zh-CN.md)为准。未在真实 8C16G/300GB 环境执行的 Gate 必须标记 `blocked`，不得标记 `passed`。
