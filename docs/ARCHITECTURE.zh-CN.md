...
本文描述当前仓库已经实现的后台、安全与限额语义，以及从 8C16G/300GB 单机拓展到 10 TB 以上独立存储集群时的目标架构。当前版本已实现九格式失败关闭解析、PostgreSQL 授权检索、带来源的可选 RAG、独立语义审核和确定性降级；尚未完成混合向量检索、10 TB 容量认证、50 亿 token/日吞吐证明及高可用生产集群验收。
...
| 文件字节 | S3 兼容对象存储；离线单机为本机 MinIO | 企业内网分布式 MinIO 或经审批的 S3 | 10 TB 以上不应由 API 本地磁盘或 PostgreSQL BLOB 承担 |
...
~~~mermaid
flowchart LR
    U["浏览器 / scripts/upload.py"] -->|"TLS: 登录、元数据、RBAC、审计请求"| E["Ingress / WAF / API Gateway"]
    E --> A["FastAPI 无状态副本"]
    A -->|"事务、账号退休、状态、审计"| P[("PostgreSQL")]
    A -->|"分布式限流"| R[("Redis")]
    A -->|"服务凭证：建分片会话、HEAD、签名"| O[("S3 / 分布式 MinIO")]
    U -->|"短期预签名 PUT/GET；不携带 JWT"| O
    A -->|"持久任务与数据库租约"| Q["Maintenance Worker"]
    Q -->|"ClamAV 流式扫描、九格式有界读取"| O
    Q -->|"扫描状态、OKF 草稿、来源索引与审计"| P
    A -->|"可选且受控的 RAG"| L["独立生成 / 审核客户端"]
~~~

控制面是 FastAPI、PostgreSQL 和 Redis；数据面是客户端与对象存储。API 只返回具有限时、对象级权限的 URL，不接收 10 TB 文件字节。这使 API 副本能按请求数横向扩容，而非按文件吞吐扩容。

当前通用 Linux 离线 Compose 包含 PostgreSQL、Redis、单节点 MinIO、MinIO Client、ClamAV、Caddy、FastAPI API/Maintenance、Migration/Bootstrap 和 Next.js Web；正式制品清单对应 PostgreSQL、Redis、MinIO、MinIO Client、ClamAV、Caddy、API、Migration、Web 9 类镜像角色。单节点 MinIO 适用于当前 300GB 单机基线，不构成 10 TB 高可用证明；未来 10 TB 目标必须使用独立多节点多盘对象存储并完成容量、故障和恢复验收。
...
sequenceDiagram
    ...
    A->>D: 校验大小；预留转已用；状态改为 quarantined / scan=PENDING
    A-->>C: FileRead(status=quarantined)
    W->>D: 使用租约领取扫描任务
    W->>S: 有界读取对象
    W->>V: 流式恶意软件扫描
    alt 扫描为 CLEAN
        W->>D: scan=CLEAN；状态改为 processing；创建 OKF 任务
        W->>S: 九格式有界解析
        W->>D: 保存 OKF DRAFT、来源定位与终态
        A->>D: 审批仅允许 CLEAN + 当前版本 OKF 成功
        A->>D: DRAFT → PUBLISHED；File → available
    else 感染、扫描异常或解析失败
        W->>D: 保持 quarantined 或记录失败终态
    end
~~~
...
7. 上传完成先进入 <code>quarantined</code>，恶意软件状态为 `PENDING`；只有 ClamAV 返回 `CLEAN` 才进入 <code>processing</code> 并创建当前版本 OKF 转换任务。
8. 具有 <code>file:approve</code> 的管理员调用 <code>POST /files/{id}/approve</code> 时，服务端再次验证 `CLEAN` 与当前版本 OKF 成功；审批发布草稿并进入 <code>available</code>。感染、扫描错误、解析失败或转换未完成均失败关闭。
...
stateDiagram-v2
    ...
    uploading --> quarantined: "完成对象、验证大小并等待扫描"
    uploading --> failed: "大小不符 / 主动中止"
    quarantined --> processing: "ClamAV CLEAN"
    quarantined --> quarantined: "感染 / 扫描异常，保持隔离"
    processing --> available: "当前版本 OKF 成功后管理员批准"
    processing --> failed: "解析 / OKF 失败终态"
...
当前权限目录至少覆盖文件读取/上传/批准、用户管理、角色读取/管理/分配、审计读取等管理动作。目录由 bootstrap 幂等写入，角色 API 只能引用目录中存在的 code。
...
| storage_bytes | bytes / lifetime | 文件发起时预留、成功完成时消费；手工知识正文写入时消费 | 文件成功上传计入全部字节；手工知识正文创建或变长仅计入 UTF-8 字节正增长 |
...
<code>storage_bytes</code> 是生命周期累计写入量，不是当前占用空间：手工知识正文同长度替换或缩小不增加计量，文件、手工知识正文的删除以及正文缩小均不返还既有额度。配置“无限制”只取消角色级额度，计量仍继续，且不能绕过平台单文件、文件数量、知识条目容量与磁盘水位等硬上限。

Redis 不可用时受保护端点 fail closed，返回 503，而不是绕过限流。Redis 官方建议在多副本服务中用共享 Redis 与原子 Lua 完成 read-decide-update。

### 5.3 账号退休

账号退休是不可逆、非物理删除的数据生命周期操作。调用 `DELETE /api/v1/users/{id}` 时，服务端重新解析操作者的动态权限与层级，并要求请求体中的 `confirmation_email` 与目标账号完全一致。系统禁止操作者退休自身或最后一个活跃超级管理员；目标账号仍拥有知识库时，必须选择处于正常状态且未退休的继任者，由同一数据库事务完成全部所有权交接。

退休事务将账号状态设为 `DISABLED`，写入 `retired_at`、`retired_by_id` 和可选原因，提升 `token_version`，撤销未失效的 refresh token 与 API Key，并移除角色分配。目标账号或其知识库存在活动外部模型请求时，撤权门禁失败关闭，不能在请求进行中改变授权或所有权。业务记录、文件归属引用和审计身份继续保留；已退休账号不能重新启用、修改密码、重新分配角色或登录。重复提交同一退休操作保持幂等，但不能恢复账号。
...
| 表 | 关键字段/约束 |
|---|---|
| users | email 唯一；status；is_superuser；token_version；role_assignment_version；retired_at/retired_by_id/retirement_reason；已退休账号必须为 DISABLED |
| roles | code 唯一；priority；is_system；policy_version |
...
| knowledge_bases | owner；外部处理策略；role_grant_version；content_version |
| knowledge_base_role_grants | knowledge_base + role 唯一；reader/editor/manager 访问级别 |
| knowledge_entries | knowledge_base；来源文件/路径；发布状态；有界正文与来源元数据 |
| files | owner；knowledge_base；私有 object_key 唯一；声明/实际大小；文件与恶意软件状态；自定义 JSON |
| okf_conversion_jobs | 文件版本；租约；尝试次数；DRAFT/PUBLISHED 结果与错误终态 |
...
| refresh_tokens | 只存 token fingerprint；令牌族；过期与撤销时间 |
| api_keys | 不可逆摘要；稳定 credential_family_id；scope；限流；轮换与撤销 |
| chat_idempotency_records | 主体/请求摘要；知识内容版本；状态；AES-GCM 密钥版本、nonce 与有界密文 |
| llm_provider_configs / llm_model_prices | 供应商加密凭据、启用状态、模型与价格版本 |
| llm_budget_policies / counters / holds | 日/月 token 与费用预算、原子预留和结算 |
| llm_usage_records | 内容无关的供应商、模型、token、费用、请求与幂等审计 |
| audit_logs | actor、action、resource、request_id、result、IP、details、时间；离线运行时角色只拥有 SELECT/INSERT |

PostgreSQL 是授权与状态事实源。Redis 只保存可重建的短时限流计数，对象存储只保存不可查询的文件字节；不要从对象 key 推断权限。

审计表内部保留 IP 与有界 details 供受控调查使用，但管理页、`GET /api/v1/audit-logs` 和 CSV 导出只投影 `id`、`created_at`、`result`、`action`、`actor_id`、`resource_type`、`resource_id`、`request_id` 八个字段，不向前端或导出文件暴露 IP/details。离线部署在每次迁移后把应用运行角色重新收敛为只读/追加审计，禁止 `UPDATE/DELETE/TRUNCATE`；这属于数据库权限边界，不等同于 hash/WORM 或远端防篡改归档。
...
| 未信任文件 → 平台 | 扩展名 allowlist、文件名 basename、平台/角色大小限制、ClamAV 失败关闭、OOXML 主动内容与 Zip Bomb 拒绝、PDF/旧 Office 断网沙箱、OKF 审批门 | 目标机黄金样本、CDR、独立沙箱池、持续病毒库与解析器供应链验收 |
...
上传文件名与 MIME 都不可信。当前 maintenance worker 已实施 ClamAV 流式扫描、九格式失败关闭解析和 OKF 审批门；感染、扫描异常、解析器缺失或转换失败都不能进入 `available`。这证明代码路径存在，不等于目标服务器的病毒库、魔数黄金集、沙箱隔离和资源上限已经通过正式运行验收；相关目标机证据缺失时仍为 `BLOCKED`。

单 PUT 已把规范 SHA-256 绑定到预签名请求，并在完成阶段执行恒定时间比对；不匹配时删除对象并释放配额。Multipart 已校验完整 part 集、ETag 与最终对象大小，但尚未为所有 S3 兼容后端建立统一的全对象 SHA-256 契约；未来强化必须统一 canonical 编码和供应商 checksum 语义，不能把 base64 `ChecksumSHA256` 与 64 位十六进制摘要直接比较。
...
1. 上传完成后进入隔离状态并创建持久任务，不在请求线程内解析；
2. maintenance worker 使用数据库租约领取任务，以有界流读取对象并调用 ClamAV；
3. TXT/CSV/OOXML 使用内建有界解析，PDF 与旧版 Office 在无网络、只读根文件系统和 CPU/内存/时限约束下调用固定工具；
4. 解析结果生成带来源定位的 OKF 草稿，当前版本转换成功且扫描为 `CLEAN` 后才允许管理员发布；
5. PostgreSQL `pg_trgm`/有界全文检索只返回已授权、已发布条目；可选 RAG 必须携带来源并通过独立审核，否则确定性降级；
6. 状态转换、租约、重试、审批、来源与模型用量写入持久化事实表或审计日志。

尚未完成的演进包括：独立消息队列/transactional outbox、混合向量检索与重排、独立扫描/解析资源池、全量孤儿对象对账，以及 10 TB 数据集下的索引吞吐和恢复验证。原始对象、解析文本、索引与未来 embedding 都必须绑定 `file_id + version`，避免覆盖后检索旧内容。

## 10. 当前生产缺口

以下项目不是“以后优化”，而是正式对外部署前的门槛：

- 真实目标机上的 ClamAV 病毒库、MIME/魔数黄金集、九格式解析沙箱和审批全链路证据；
- 全量孤儿对象扫描、transactional outbox 与跨区域/跨账户存储对账（过期 UploadSession、HELD reservation 和 `FINALIZING` 基础恢复已实现）；
- 文件删除、保留策略和法律保全流程；如业务未来改为“删除返还”，必须另行实现 <code>storage_bytes</code> 对账与退款流程；
- 用户级 limit override 管理接口和角色分配到期管理；密码修改/重置、角色 CAS 编辑与安全删除、不可逆账号退休、审计查询及有界脱敏导出已经实现；
- Multipart 跨 S3 兼容后端的统一全对象 checksum 契约；单 PUT SHA-256 已强制执行；
- PostgreSQL HA/PITR、对象版本/复制、Redis HA 与定期恢复演练；
- 指标、trace、集中日志、审计日志防篡改归档和告警；
- 可选 RLS、HA 和 PITR；离线部署已分离运行账户与迁移 owner，但仍需目标机权限和恢复证据；
