...
4. 目标机使用预置公钥验签后，临时启动只绑定 `127.0.0.1:5000` 的只读 Registry。为兼容 Docker 29，临时 bridge 不使用会屏蔽宿主发布端口的 `--internal`，而是显式关闭 masquerade 与 ICC，在启动闸门打开前通过共享 network namespace 删除 Registry 默认路由并复核只剩本地直连路由。每次 `docker pull --platform linux/amd64` 前先复算 Registry manifest/config 原始字节的摘要、大小与平台，pull 后再验证精确 RepoDigest；完成后立即删除精确标记的容器和网络。
...
`bundle.control` 只允许下面八个字段且不得重复。Bootstrap Registry 镜像的运输 tar **不在本 bundle 内，也不由导入脚本验签**；它是建立回环 Registry 之前的独立信任前置。安全管理员必须先在隔离流程中核验 tar 的独立签名与 SHA-256，再将其载入本地 Docker。导入脚本随后从本机镜像描述符或单镜像 archive 复算真实 config digest，并与已签署的 config image ID 及 `linux/amd64` 平台核对；启动时使用核对后由当前 Docker 后端返回的精确本机内容 ID，而不再依赖可变运输标签。必须保留 tar 验签记录，并由发布方签署以下完整控制身份与构建侧实测容量上界（尖括号占位符必须替换为真实值）：

```dotenv
REGISTRY_BOOTSTRAP_IMAGE=heyi-bootstrap/registry:2.8.3-amd64-<transport-id-prefix>
...
目标机 `/var/tmp` 必须是 `root:root`、模式 `1777` 的真实目录，并具备容纳“最大单个本机镜像 archive”的临时空间。导入器通常从 Docker descriptor 直接取得 config digest；当某个 image store 不提供该注解时，才会在 `/var/tmp` 创建一个 `0600`、随机命名、单次使用的 archive，复算 config 内容地址与平台并立即删除。该兼容性回退不承担 RepoDigest 运输职责，也不得把 archive 复制到 COS 或其他共享位置。

目标机固定发布公钥路径为 `/etc/heyi-release/trusted-release-public.pem`，文件必须为 root 所有、`0400/0444`、非符号链接、单硬链接的普通文件。当前批准的公钥文件 SHA-256 为 `a927ab4fdfe9febd10a5aeb5b78507ddfe7734e2fb46131524eaca00162967fa`；操作员必须从独立 CMDB/密钥托管渠道复核该指纹，不能从制品目录、COS、`.sha256` 文件或聊天记录取得。公钥轮换必须单独审批并同步更新本节；普通发行不得覆盖固定信任根。

Linux 目标机必须先对 Bootstrap tar 与主 bundle tar **分别**验证外层 checksum 签名，再分别执行 `sha256sum --check --strict`。只有四项全部成功，才能加载 Bootstrap、解包主 bundle 并调用导入器。下面命令可整体复制；先把 `RELEASE_ID` 替换为本次已批准发行号，并由 root 将六个运输文件放入发行专属目录：
...
`import-offline-registry-bundle.sh` 验证的是主 bundle **内部**的 `SHA256SUMS.sig` 与内容合同；它不会验证两个运输 tar 的外层 `.sha256.sig`，也不会替代 Bootstrap tar 的外层验签。跳过上面任一条 `openssl dgst` 或 `sha256sum --check --strict` 命令都必须判定为 **FAIL / NO-GO**。
...
导入脚本拒绝符号链接、非绝对路径、非 root 所有路径、组/其他用户可写祖先、路径穿越、签名或逐文件摘要不符、发布文件不一致、发布重放/降级、端口被占用、Registry 原始 manifest/config 字节不符、非 `linux/amd64`、config digest 不符和 RepoDigest 不符；清理时只删除具有本项目 owner/purpose 标签且 ID 完全一致的临时容器和隔离网络。全部镜像验证完成后，它先持久化并同步精确 Registry 回执及其目录，再原子推进并同步 `highest-release.json`，因此在任一提交窗口断电后都可以安全重试。若最高序号已经等于本次发行，导入器不会创建网络、容器或再次拉取镜像，而是重新验证完整签名发布状态和九个本机镜像后返回审计 no-op；同序号状态不完整或不一致时要求人工复核。缺失、不安全、不是最高已接受发布或与目标发布不一致的收据会阻断安装/升级。它不会执行 `compose down`、不会清理其他镜像，也不会访问公网。
...
export BUNDLE_ROOT="/srv/heyi-knowledgebases-offline/artifacts/${RELEASE_ID}/offline-registry-bundle"
...
```bash
export RELEASE_ID='<release-id>'
export TARGET_BUNDLE="/srv/heyi-knowledgebases-offline/artifacts/${RELEASE_ID}/offline-registry-bundle"
