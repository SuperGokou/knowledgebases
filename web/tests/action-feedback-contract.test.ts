...
describe("action feedback source contract", () => {
  ...
  it("auto-dismisses success while keeping failures visible until acknowledged", () => {
    expect(feedback).toContain("const SUCCESS_DURATION_MS = 6_000");
    expect(feedback).toContain(': input.tone === "info"');
  ...
  it("uses lightweight motion and honors the existing reduced-motion guard", () => {
